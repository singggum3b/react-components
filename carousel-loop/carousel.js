...
var CarouselLoop = React.createClass({
	...
	componentWillMount() {
		this._nextFunc = $.throttle(this.props.animationTimeout,(force)=>this.next(force));
		this._prevFunc = $.throttle(this.props.animationTimeout,(force)=>this.prev(force));
	},
	componentDidMount() {
	...
	setActiveGroup(index, direction) {
		...
		let groupedIndex = this.getGroupedItemIndex(this.props, this.state);

		var _targetIndex = index;
		let _newAnchorIndex = this.state.anchorIndex;

		if (index >= groupCount) {
			_targetIndex = 0;
			_newAnchorIndex = groupedIndex.last().last() + 1;
			_newAnchorIndex = _newAnchorIndex > (childCount - 1) ? 0 : _newAnchorIndex;
		} else if (index < 0) {
			_targetIndex = groupCount - 1;
			_newAnchorIndex = groupedIndex.first().first() + groupCount * (childCount - this.props.visibleItemCount);
		}

		//console.log(_targetIndex,_newAnchorIndex);
	...
	getGroupedItemIndex(props, state) {
		...
		return Immutable.Range(0, groupCount).toList().map((groupNumber, index)=> {
			if (childCount <= visibleItemCount) {
				return Immutable.Range(0, childCount);
			} else {
				return Immutable.Range(visibleItemCount * groupNumber + state.anchorIndex, visibleItemCount * (groupNumber + 1) + state.anchorIndex).toList();
			}
		}).map((itemGroup, index)=> {
			return itemGroup.map((itemIndex, idx)=> {
				//console.log(itemIndex);
				if (itemIndex >= childCount) {
					return (itemIndex - childCount) % childCount;
				} else if (itemIndex < 0) {
					return (itemIndex + childCount) % childCount;
				} else {
					return itemIndex;
		...
		return React.Children.count(props.children) ? (
			<div className={_className}>
				...
				<ReactCSSTransitionGroup component={Swipeable}
																 ...
																 transitionAppearTimeout={props.animationTimeout}
																 transitionEnterTimeout={props.animationTimeout} transitionLeaveTimeout={props.animationTimeout}
...
CarouselLoop.Item = React.createClass({
