			...
			<tr key={rowIndex}>
				{
					row.map((value, index)=> {
						...
						return this.props.centerHeader ? (
							index == _center ? MasterPage.buildHeader(value, index) : this.props.buildCell(value, index)
						) : (
							index == 0 ? MasterPage.buildHeader(value, index) : this.props.buildCell(value, index)
						)
					})
				}
			</tr>
	...
	buildHorizontalHeader(row) {
		return (
			...
			<tr>
				{
					row.map((value, index)=> {
						return MasterPage.buildHeader(value, index)
					})
				}
			</tr>
			</thead>
