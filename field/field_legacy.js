...
var Field = React.createClass({
	...
	getValidationMsg() {
		...
		let _result = (this.state.activeValue && this.props.getValidationMsg && this.props.validation) ? this.props.validation.map((vld,index)=>{
			let _isValid = false;
			try {
	...
	onChange(e) {
		...
		let _activeValue = match(e.target.type, {
			"checkbox": ()=> {
	...
	buildComponent(props,state) {
		return (
			<label className={this.className(this.props,this.state)}>
				{this.props.title}p
				<input key={state.fileInputKey} accept={props.accept} maxLength={props.maxLength} tabIndex={props.tabIndex} required={props.required} type="file"
